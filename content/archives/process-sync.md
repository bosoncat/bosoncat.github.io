...
以上的代码均以好玩，简单为目的。Go 真的很有趣，可以像上面那样用类似伪码的形式把进程同步的小问题解决掉，再加上它的运行速度 (尽管 1.3 自举后慢了一些，但还是很快)，我很喜欢。具体 `goroutine` 以及 `go channel` 的用法和原理可以到 [golang.org](https://golang.org) 参考。
